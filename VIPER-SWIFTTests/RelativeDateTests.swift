...
class RelativeDateTests: XCTestCase {
    ...
    func testDateBeforeTodayIsOutOfBounds() {
        let date = calendar.dateWithYear(year:2000, month: 1, day: 1)
        let today = calendar.dateWithYear(year:2000, month: 1, day: 2)
    ...
    func testTodayRelatesAsToday() {
        let date = calendar.dateWithYear(year:2000, month: 1, day: 1)
        let today = calendar.dateWithYear(year:2000, month: 1, day: 1)
        let relation = calendar.nearTermRelationForDate(date, relativeToToday: today)
    ...
    func testTomorrowDuringTheSameWeekAsTodayRelatesAsTomorrow() {
        let date = calendar.dateWithYear(year:2004, month: 1, day: 3)
        let today = calendar.dateWithYear(year:2004, month: 1, day: 2)
        let relation = calendar.nearTermRelationForDate(date, relativeToToday: today)
    ...
    func testTomorrowDuringNextWeekRelatesAsNextWeek() {
        let date = calendar.dateWithYear(year:2004, month: 10, day: 10)
        let today = calendar.dateWithYear(year:2004, month: 10, day: 9)
        let relation = calendar.nearTermRelationForDate(date, relativeToToday: today)
    ...
    func testDateAfterTomorrowButDuringTheSameWeekAsTodayRelatesAsLaterThisWeek() {
        let date = calendar.dateWithYear(year:2004, month: 3, day: 6)
        let today = calendar.dateWithYear(year:2004, month: 3, day: 2)
        let relation = calendar.nearTermRelationForDate(date, relativeToToday: today)
    ...
    func testDateDuringWeekAfterTodayRelatesAsNextWeek() {
        let date = calendar.dateWithYear(year:2004, month: 7, day: 11)
        let today = calendar.dateWithYear(year:2004, month: 7, day: 6)
        let relation = calendar.nearTermRelationForDate(date, relativeToToday: today)
        XCTAssertEqual(relation, NearTermDateRelation.NextWeek, "\(date) should be next week.")
    }
    
    func testDateDuringWeekAfterTodayThatFallsInNextYearRelatesAsNextWeek() {
        let date = calendar.dateWithYear(year:2006, month: 1, day: 2)
        let today = calendar.dateWithYear(year:2005, month: 12, day: 29)
        let relation = calendar.nearTermRelationForDate(date, relativeToToday: today)
        XCTAssertEqual(relation, NearTermDateRelation.NextWeek, "\(date) should be next week.")
    }
    
    func testDateBeyondNextWeekRelatesAsOutOfRange() {
        let date = calendar.dateWithYear(year:2005, month: 9, day: 25)
        let today = calendar.dateWithYear(year:2005, month: 9, day: 14)
        let relation = calendar.nearTermRelationForDate(date, relativeToToday: today)
